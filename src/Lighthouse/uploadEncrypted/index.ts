...
export default async (sourcePath: any, apiKey: string, publicKey: string, signed_message: string) => {
  try {
    ...
    if (stats.isFile()) {
      ...
      const { error } = await saveShards(publicKey, response.data.Hash, signed_message, keyShards);
      if (error) {
