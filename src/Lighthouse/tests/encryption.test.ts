...
describe('EncryptionTest', () => {
  test('fetchEncryptionKey Main Case File', async () => {
    ...
    const response = await lighthouse.fetchEncryptionKey(cid, publicKey, signed_message);
  ...
  test('fetchEncryptionKey Not Authorized', async () => {
    try {
      ...
      const signed_message = '0xd7f1e7ccf6e3620327d3b29c57018d076305148eec487c57d8121beac0067895';

      const key = await lighthouse.fetchEncryptionKey(cid, publicKey, signed_message);
    } catch (error: any) {
  ...
  test('getAuthMessage main', async () => {
    const response = await lighthouse.getAuthMessage('0x1Ec09D4B3Cb565b7CCe2eEAf71CC90c9b46c5c26');
  ...
  test('Access Control', async () => {
    ...
    const response = await lighthouse.accessCondition(publicKey, cid, signedMessage, conditions, aggregator);
  ...
  test('Revoke Access', async () => {
    ...
    const response = await lighthouse.revokeFileAccess(publicKey, revokeTo, cid, signedMessage);
