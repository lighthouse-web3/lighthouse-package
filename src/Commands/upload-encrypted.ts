...
const getQuote = async (path: string, publicKey: string, Spinner: any) => {
  ...
  for (let i = 0; i < quoteResponse.data.metaData.length; i++) {
    console.log(
      quoteResponse.data.metaData[i].fileName +
        ...
        Array(12 - bytesToSize(quoteResponse.data.metaData[i].fileSize).toString().length)
          .fill('\xa0')
          .join('') +
  ...
  console.log('\r\n' + chalk.cyan('Summary') + '\r\nTotal Size: ' + bytesToSize(quoteResponse.data.totalSize));
  ...
  console.log(
    'Data Limit: ' +
      ...
      '\r\nAfter Upload: ' +
      bytesToSize(
        parseInt(quoteResponse.data.dataLimit) - (parseInt(quoteResponse.data.dataUsed) + quoteResponse.data.totalSize),
      ),
  );
...
const uploadFile = async (path: string, signer: any, apiKey: string, network: any) => {
  const spinner = new Spinner('Uploading...');
  spinner.start();

  const messageRequested = (await lighthouse.getAuthMessage(config.get('LIGHTHOUSE_GLOBAL_PUBLICKEY') as string)).data
    .message;
  const signedMessage = await signer.signMessage(messageRequested);
  const uploadResponse = (
    await lighthouse.uploadEncrypted(path, apiKey, config.get('LIGHTHOUSE_GLOBAL_PUBLICKEY') as string, signedMessage)
  ).data;
  ...
  console.log(
    chalk.green('File Uploaded, visit following url to view content!\r\n') +
      chalk.cyan('Visit: ' + 'https://files.lighthouse.storage/viewFile/' + uploadResponse.Hash),
  );
...
export default async function (_path: string) {
  if (!_path) {
    ...
    try {
      ...
      // Display Quote
      const quoteResponse = await getQuote(path, config.get('LIGHTHOUSE_GLOBAL_PUBLICKEY') as string, Spinner);

      // Upload
      console.log(
        chalk.green('Carefully check the above details are correct, then confirm to complete this upload') + ' Y/n',
      );
      ...
      const selected: any = await Read(options, (err, res) => console.log(err, res));

      if (selected.trim() === 'n' || selected.trim() === 'N' || selected.trim() === 'no') {
