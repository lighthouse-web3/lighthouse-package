...
const getQuote = async (path: string, publicKey: string, Spinner: any) => {
  ...
  for (let i = 0; i < quoteResponse.metaData.length; i++) {
    console.log(
      quoteResponse.metaData[i].fileName +
        ...
        Array(12 - bytesToSize(quoteResponse.metaData[i].fileSize).toString().length)
          .fill('\xa0')
          .join('') +
  ...
  console.log('\r\n' + chalk.cyan('Summary') + '\r\nTotal Size: ' + bytesToSize(quoteResponse.totalSize));
  ...
  console.log(
    'Data Limit: ' +
      ...
      '\r\nAfter Upload: ' +
      bytesToSize(parseInt(quoteResponse.dataLimit) - (parseInt(quoteResponse.dataUsed) + quoteResponse.totalSize)),
  );

  const remainingAfterUpload =
    parseInt(quoteResponse.dataLimit) - (parseInt(quoteResponse.dataUsed) + quoteResponse.totalSize);
...
const uploadFile = async (path: string, signer: any, apiKey: string, network: string) => {
  ...
  console.log(
    chalk.green('File Uploaded, visit following url to view content!\r\n') +
      chalk.cyan('Visit: ' + 'https://gateway.lighthouse.storage/ipfs/' + uploadResponse.Hash + '\r\n') +
      chalk.cyan(Array(7).fill('\xa0').join('') + 'https://ipfs.io/ipfs/' + uploadResponse.Hash),
  );
...
export default async function (_path: string) {
  if (!_path) {
    ...
    try {
      ...
      const password: any = await Read(_options, (err, res) => console.log(err, res));
