...
export default async function (data: any, options: any) {
  if (JSON.stringify(data) === '{}') {
    ...
    try {
      const dealStatus = (await lighthouse.dealStatus(data.cid)).data.dealStatus;
      dealStatus.length === 0 ? console.log('Deal creation in progress') : showResponse(data.cid, dealStatus);
    } catch (error: any) {
