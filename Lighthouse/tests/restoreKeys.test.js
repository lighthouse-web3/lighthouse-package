...
test("restoreKeys", async () => {
  const responseKey = await lighthouse.restoreKeys(
  ...
  expect(responseKey).toHaveProperty("privateKeyEncrypted");
  expect(responseKey).toMatchObject(expectedKey);
});
