...
exports.user_token = async (expiry_time) =>{
  try{
    const response = await axios.get(URL+'/api/estuary/user_token?expiry_time='+ expiry_time);
    return response.data;
  } catch{
    return null;
  }
...
exports.push_cid_tochain = async(privateKey, cid) => {
  try{
    ...
    const response = await axios.post(URL + `/api/estuary/push_cid_tochain`, body);
    return response.data;
  } catch{
    return null;
  }
